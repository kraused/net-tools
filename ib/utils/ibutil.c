...
static void fillMAD_Get_LFTRecord(char *umad, SInt64 len, UInt16 smLid, UInt16 destLid)
{
	UInt64 mask;
...
static void fillMAD_Get_NodeRecord(char *umad, SInt64 len, UInt16 smLid, UInt16 destLid)
{
	UInt64 mask = 0;
                                        ...
                                        SInt32 *numLFTs, struct LinearForwardingTable **LFTs)
{
	...
	fillMAD_Get_LFTRecord(umad, sizeof(umad), smLid, 0);
	...
	j = 0;
	for (i = 0 ; i < n; ++i) {
		p  = (struct _LinearForwardingTableRecord *)(umad_get_mad(buf) + IB_SA_DATA_OFFS + i*(offset << 3));
		j += (0 == p->block);
	}
                                        ...
                                        UInt16 lid, struct LinearForwardingTable *LFT)
{
	...
	fillMAD_Get_LFTRecord(umad, sizeof(umad), smLid, lid);
                              ...
                              SInt32 *numNRs, struct NodeRecord **NRs)
{
	...
	fillMAD_Get_NodeRecord(umad, sizeof(umad), smLid, 0);
	...
	*NRs    = alloc(allocUd, NULL, 0, (*numNRs)*sizeof(struct NodeRecord));
	...
	for (i = 0 ; i < n; ++i) {
		p = (struct _NodeRecord *)(umad_get_mad(buf) + IB_SA_DATA_OFFS + i*(offset << 3));
                             ...
                             UInt16 lid, struct NodeRecord *NR)
{
	...
	fillMAD_Get_NodeRecord(umad, sizeof(umad), smLid, lid);
	...
	p = (struct _NodeRecord *)(umad_get_mad(buf) + IB_SA_DATA_OFFS + 0*(offset << 3));
